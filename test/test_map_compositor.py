#!/usr/bin/python
# coding=utf-8
"""Tests for MapCompositorSlots source resolution.
...
class TestMapCompositorSourceResolution(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.test_dir = tempfile.mkdtemp(prefix="map_compositor_test_")
    ...
    def _slots_with_source(self, text):
        ...
        inst = MapCompositorSlots.__new__(MapCompositorSlots)
        inst.ui = Mock()
    ...
    def test_split_source_directory_is_empty(self):
        ...
        self.assertEqual(MapCompositorSlots._split_source(self.maps_dir), [])

    def test_split_source_single_file(self):
        path = self.files["mat_BaseColor.png"]
        self.assertEqual(MapCompositorSlots._split_source(path), [path])

    def test_split_source_multiple_files(self):
        self.assertEqual(
            MapCompositorSlots._split_source(self.joined), list(self.files.values())
        )

    def test_split_source_empty(self):
        self.assertEqual(MapCompositorSlots._split_source(""), [])

    # -- _field_dir -------------------------------------------------------

    def test_field_dir_directory(self):
        self.assertEqual(MapCompositorSlots._field_dir(self.maps_dir), self.maps_dir)

    def test_field_dir_single_file(self):
        path = self.files["mat_Roughness.png"]
        self.assertEqual(MapCompositorSlots._field_dir(path), self.maps_dir)

    def test_field_dir_joined_files(self):
        self.assertEqual(MapCompositorSlots._field_dir(self.joined), self.maps_dir)

    def test_field_dir_empty(self):
        self.assertEqual(MapCompositorSlots._field_dir(""), "")

    # -- _validate_source -------------------------------------------------

    def test_validate_source_directory(self):
        self.assertTrue(MapCompositorSlots._validate_source(self.maps_dir))

    def test_validate_source_single_file(self):
        self.assertTrue(
            MapCompositorSlots._validate_source(self.files["mat_Metallic.png"])
        )

    def test_validate_source_multiple_files(self):
        self.assertTrue(MapCompositorSlots._validate_source(self.joined))

    def test_validate_source_nonexistent(self):
        self.assertFalse(
            MapCompositorSlots._validate_source(os.path.join(self.maps_dir, "nope.png"))
        )
    ...
    def test_validate_source_mixed_valid_and_missing(self):
        ...
        self.assertFalse(MapCompositorSlots._validate_source(text))

    def test_validate_source_empty(self):
        self.assertFalse(MapCompositorSlots._validate_source(""))
